...
- Your page has to let someone about to change this component answer four questions from the page alone, so establish propositions that answer all four in specifics: what it is responsible for and deliberately is not; where it lives, down to packages, files, and named entrypoints; what crosses its boundary in each direction, including the data or contract that passes; and how someone would check they had not broken it, naming the focused tests, the behaviours they prove, and the commands that run them. Then cover what else the subject requires: important symbols, invariants and lifecycle ordering, extension points, schemas, and configuration.
- Name the thing rather than the category. "Covered by unit tests" answers nothing; "TestQueueRunPayload proves an empty hash_key is rejected before any Redis write" answers it. A page that established two or three propositions has not read its subject.
