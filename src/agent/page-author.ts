...
- State what the component does NOT do, alongside what it does. An invariant is usually a refusal: a host that is not reachable from outside, a header rebuilt from an allowlist rather than forwarded, a caller-supplied map that is copied rather than mutated, a plane that never calls back into the other. These are the facts a reader most needs before changing something and the ones least visible in the code, because nothing in the source says "and not this" - you establish them by noticing what the implementation refuses to accept and what its tests assert cannot happen.
- Say what happens when it fails, not only when it works: which error the caller sees, what is retried, what is left behind, what a partial write leaves in the store. And say what is exempt - the case that skips the check, the client that bypasses the gate, the configuration that collapses the normal path onto a fallback.
