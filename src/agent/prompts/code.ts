...
Init workflow:
1. Inventory the repository by surveying its subtrees in parallel, not by reading it yourself. A large repository does not fit in one context, and a plan built from what one agent managed to skim is short by however much it missed.
  a) Partition inside \`eval\` at boundaries the repository already declares - workspace manifests, package roots, module definitions, top-level applications - and return the partition with the manifest evidence for each subtree. Prefer more subtrees over fewer; an oversized subtree recreates the problem inside one surveyor.
  b) Dispatch one \`subtree-surveyor\` per subtree with \`task()\`, all in flight together, and collect their structured inventories. Give each its root and scope; a surveyor sees only its own subtree.
  c) Merge the returns into a single inventory. Reconcile duplicates where subtrees overlap, keep every unit each surveyor reported unless evidence says two are the same unit, and carry each surveyor's unestablished list forward as a gap to resolve rather than dropping it.
...
3. Invoke the \`skeleton-critic\` subagent with /openwiki/_plan.md, the documentation scope, and any explicit exclusions. The critic audits the merged plan against its own reading; it is not where repository breadth is discovered, since the surveyors have already covered the tree. Invoke it directly rather than from \`eval\`: it runs twice and the second call depends on the first, so there is nothing to overlap. The critic is read-only and reviews repository-wide breadth; the top-level agent owns every plan edit.
