...
6. After completing the wiki skeleton and resolving every critic TODO, fill the contents for every page in the skeleton. A passing mention, directory list, source-map row, or concise overview is not substantive coverage: explain responsibilities, owning entrypoints and symbols, important relationships and invariants, focused tests, and primary evidence when they exist.
  a) For each system or workflow page, trace the evidence-backed path end to end when the source supports it: the initiating entrypoint, ordered calls or state transitions, data or control crossing module boundaries, configuration or precedence decisions, failure propagation and recovery behavior, and the focused tests that establish those details.
  b) Use 500–1,000 words of body prose as the normal depth expectation for a substantive page, and write more when the system's complexity warrants it. Before verification, inspect per-page word counts and expand any substantive page below 500 words when meaningful source-grounded behavior remains undocumented. If a concept is genuinely too narrow for that depth, merge it into the closest related canonical page instead of padding it.
  c) Length never justifies repetition, speculation, invented guarantees, unsupported claims, or secret-bearing examples. Every added detail must improve the reader's source-grounded understanding.
  d) REMEMBER: An agent or human should be able to use the wiki to fully understand the codebase and its systems/workflows without needing to read a single line of code outside of the wiki.
...
8. Before finishing, reconcile the final wiki tree against the full inventory. Verify coverage, source grounding, terminology, navigation, relationship links, and the substantive-page depth audit. Creating every skeleton file is not completion; continue until the planned pages satisfy this reconciliation.
...
- Do not document every file or target a fixed page count. Wiki depth and page length should reflect meaningful repository complexity.
