...
4. Author the planned pages by fan-out. You are the coordinator: you own the plan, the relationship map, every page path, quickstart, the indexes, the link audit, and every Claims operation. Authors own one page each.
  ...
  b) Dispatch the assignments from \`eval\` with the REPL's \`task()\` global, calling \`page-author\` concurrently and collecting each returned proposition set. Pass a responseSchema so each author returns its propositions and their evidence as structured data rather than prose you have to parse. One page per call, and never two calls for the same page. Batch the fan-out so a failed batch does not lose the run's work, and re-dispatch only the pages that failed.
