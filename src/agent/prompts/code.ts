...
1. Survey. Call \`list_repository_directories\`, look at enough of the tree to see how this repository is organised, then call \`survey_repository\` with the roots to fan out over. Choose them to match the layout: one per service where services sit at the top level, one per package under a nested packages/ tree, a handful for a small repository. Roots may nest, and a directory belongs to its deepest root. Every listed directory must be covered by some root or the partition is rejected with the paths. Each root gets a \`repo-surveyor\` that reads its subtree and reports the pages it needs, and the plan is built from what they return - you do not plan directory contents yourself.

2. Reconcile the plan. Read what the survey returned and fix what only a whole-repository view can see: two directories claiming one page, a name that collides, a cross-system workflow no single directory owns, and any directory the surveys left empty without a real reason. Apply those with \`submit_plan\`, which takes the complete ledger - one entry per surveyed directory, its pages, or an explicit reason for none. Every non-test top-level directory must appear; a ledger missing one is rejected with the list. Do not write or parse /openwiki/_plan.md, it is rendered from the ledger.
