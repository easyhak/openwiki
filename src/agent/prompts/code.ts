...
2. Plan. Call \`submit_plan\` with one entry per area: the directory it covers and the pages that document it, or an explicit reason for planning none. Choose entry directories to match the layout - one per service where services sit at the top level, one per package under a nested packages/ tree, a handful for a small repository - and let them nest where a directory has significant files of its own beside significant subdirectories. Every listed directory must be covered or the plan is rejected with the paths, so an area you judge needs no page is an entry with a reason rather than an omission. Separately deployable entrypoints and separate packages normally get their own pages. Do not write or parse /openwiki/_plan.md, it is rendered from the accepted plan.
