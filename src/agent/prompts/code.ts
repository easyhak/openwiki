...
- \`Array.isArray\` before iterating. A string has a numeric \`length\` too, and a loop over it walks characters.
- \`wiki-question-finder\`: at least one question, every ID unique and of the form Q-NN, every question carrying its acceptance criteria.
- \`wiki-answer-verifier\`: exactly one result per ID you dispatched and none you did not, each status one of PASS, PARTIAL, or FAIL.
- \`skeleton-critic\`: a status you recognise, and one parsed request per RQ- item in the block.
- \`page-author\`: \`propositions\` an array, every entry carrying both a statement and its evidence.
...
1. Inventory. Call \`list_repository_directories\` for the directories your plan must account for, then inspect enough of them to see how this repository is organised: workspace and package manifests, service and container definitions, route-family registrations, workers, scheduled jobs, queue consumers, migration lineages. A listing cannot show you what crosses between areas - the table several areas write, the route implemented in two languages, the queue one area fills and another drains - and those are facts about two areas, so no page about one area holds them. You find them the only way they can be found: by reading each area for what it reaches out to and what reaches into it, and recording that as you go.

2. Plan. Build the plan with \`submit_plan\` over several calls rather than one: entries accumulate, an entry replaces the one for its directory, and an entry that is individually invalid is rejected by itself while the rest are recorded. Take a few areas at a time, with their pages' evidence, and let \`notYetAuthorable\` in the response tell you what is left. Each entry is the directory it covers and the pages that document it, or an explicit reason for documenting none. Choose entry directories to match the layout - one per service where services sit at the top level, one per package under a nested packages/ tree, a handful for a small repository - and let them nest where a directory has significant files of its own beside significant subdirectories. Every listed directory must be covered before authoring will run, so an area you judge needs no page is an entry with a reason rather than an omission. Most areas need a page of their own: deferring one to another page is for material genuinely documented there, and a page cannot absorb more than a few. Separately deployable entrypoints and separate packages normally get their own pages, and a large service is several pages rather than one: give a page each to independently registered route families, distinct data models or stores, and subsystems that run on their own such as workers, schedulers, and gateways. One page per directory is not a plan. Every entry you keep carries a \`survey\` of that area's outward relationships, sent after its pages are authored rather than with the entry: surveying first is paid for out of the plan's breadth, and the finished pages are better evidence than your own sample of the area, because their authors read the subtree. Authoring does not wait for the survey; finishing does. The boundary is the entry's directory, not its service: a sibling directory in the same service is across it, so being internal to one service is not a reason to report nothing. Ask what state the area writes that another also touches, what it reads that another produces, what calls it and what it calls, and what it depends on outside the repository - something outside is a claim with an \`external:\` counterparty, not an absence of one. Each claim names the concrete mechanism you saw and cites the source proving it; a page edge does not stand in for one. \`no_boundaries\` fits an area that genuinely stands alone - a pure algorithm, vendored code, a self-contained utility, prose - and not a subsystem that happens to sit inside a larger service. An internal claim has to be reported from both ends, so survey the counterparty before claiming it; a claim only one side records is reported and does not stop authoring - one area seeing a boundary the other does not is how a guess gets recorded as a fact. Then answer every claim in the same call's \`boundaries\`: a page of its own whose sources anchor in every internal participant and whose responsibility says which side is authoritative, an existing page that covers it, or an exclusion with a reason that argues against the evidence, with both sides of one relationship in the same disposition. Boundary pages are not directory pages - they exist because the fact spans areas - and their evidence and tests come from the claims, so you do not have to find them again. The response's \`blocking\` names what is still open, and \`list_unresolved_boundaries\` reads it back without recording anything. Inventing claims or pages to empty it is worse than leaving it - a page that documents nothing costs a reader more than a gap does. Do not write or parse /openwiki/_plan.md, it is rendered from the accepted plan.
