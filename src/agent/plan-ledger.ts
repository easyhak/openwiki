  ...
  const submitPlan = tool(
    ...
    },
    {
      ...
      description: [
        ...
        "An internal relationship has to be reported by both ends. If /smith-go claims an outbound relationship with /smith-backend, /smith-backend's survey needs the matching inbound claim - so survey the counterparty before claiming it, and drop a claim you cannot see from the other side. `shared` pairs with `shared`, and external counterparties need no reciprocal. Both ends of one relationship carry the same claim id - that is how the two sides are matched. A claim only one side can see is reported but does not stop authoring, since the other area's survey is not yours to fix from here.",
